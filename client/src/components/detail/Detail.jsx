...
import { getDetail, clearDetail, removeOwnership } from '../../redux/actions.js';
...
const price = convertir();
    ...
    return (
        ...
        <p className='p'>Cochera:&nbsp;{ownership.garage === true ? 'Tiene' : 'No tiene'}</p>
        ...
        <h3>Comentarios:<br/>{ownership.review?.map((rev, index) => (<p key={index}>{rev}</p>))}</h3>
        <p className='p'>Direccion:&nbsp;{ownership.address}</p>
        <h5 className='images'>Imagenes:&nbsp;<br/>{ownership.images?.map((imgSrc, index) => (<img src={imgSrc} key={index} alt={index+'img'}/>))}</h5>
        {
            user.role >= 3 ? <button onClick= {handleDelete} className='bt'>Remove ownership</button> : null
        }
                </div>
                :
                <div className='loading'>
                    <img src='https://www.bel-pa.com.tr/wp-content/plugins/wp-multi-store-locator-pro/assets/img/loader.gif' alt='..loading'/>
                </div>
            }
        </div>
    )
